...
def pair_min_distance(molA: Mol, molB: Mol):
    ...
    coords1 = np.array([list(confA.GetAtomPosition(i)) for i in range(molA.GetNumAtoms())])
    coords2 = np.array([list(confB.GetAtomPosition(i)) for i in range(molB.GetNumAtoms())])
