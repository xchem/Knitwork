...
def get_subnodes(
    smiles: str, terminal_subnodes: bool = CONFIG["FRAGMENT_TERMINAL_SUBNODES"]
) -> list:
    """
    ...
    terminal_subnode_query = """
        ...
        RETURN f
        """
    subnode_query = "MATCH (fa:F2 {smiles: $smiles})-[e:FRAG*0..20]->(f:F2) RETURN f"
    ...
    with DRIVER.session() as session:
        subnodes = [
            record["f"]["smiles"] for record in session.run(query, smiles=smiles)
        ]
